...
export function AccountProvider({ children }: AccountProviderProps) {
        ...
        try {
          ...
          if (tokenParts.length === 3) {
            ...
            if (payload.exp && payload.exp * 1000 > Date.now()) {
              console.log('🔄 Restoring authentication from token:', payload.address || payload.sub);
              ...
              dispatch({
                ...
                payload: {
                  ...
                  ownerAddress: userData?.ownerAddress || payload.address || payload.sub,
              ...
              // Mark as registered since we have a valid token
              dispatch({
  ...
  const generateRegistrationMessage = useCallback((walletAddress: string): string => {
  ...
  const registerUser = useCallback(async (walletAddress: string, message: string, signature: string) => {
  ...
  const loginUser = useCallback(async (walletAddress: string, message: string, signature: string) => {
    const apiUrl = process.env.NEXT_PUBLIC_API_AGENT_URL;
    if (!apiUrl) {
      throw new Error('NEXT_PUBLIC_API_AGENT_URL is not configured');
    }
  ...
  const checkRegistration = useCallback(async (walletAddress: string) => {
    ...
    try {
      ...
      dispatch({ 
        type: 'LOGIN_FAILURE', 
        payload: error instanceof Error ? error.message : 'Registration check failed' 
      });
    }
  }, []);
  ...
  }, [isConnected, address, connector, checkRegistration, state.user?.id, state.isAuthenticated]);
...
export function useWallet(): { address: string | undefined; isConnected: boolean; connector: any } {
  const { address, isConnected, connector } = useAccount();
